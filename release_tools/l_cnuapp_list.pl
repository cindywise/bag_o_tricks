...
usage: $0 [OPTIONS] 
    ...
    -r, --role          =[master|slave|snapshot]  Output only clusters serving that role. Default is to include just master and slave.
    -l, --local         Output psql commands that will connect local on the server
...
my @return = `clusterinfo_parse.pl --active_only --cluster=cnuapp --role=$in_role --pnl=$in_pnl`;
...
{
  my ($machine, $cluster, $pnl) = split(" ", $return_line);
  if (($in_pnl ne "") 
          ...
          and $pnl ~~ @standard_list))
     {
         ...
         } else 
         {
          ...
          if ($local_psql)
             {
              print STDOUT "ssh $machine \"psql --cluster $cluster -d $database\"\n";
             } else
             { 
